...
<script>
	$(document).ready(function() {
		$('#slider1').on('change', function(){
			$.ajax({url: "/web_project/Controller?link=main_room&action=update&s_id=" + <%=bean.getS_aid()%> + "&sensor=led&state=" + $('#slider1').val() , success: function(result){
		   //     alert(result);
		    }});
			
		});
		$('#slider2').on('change', function(){
			$.ajax({url: "/web_project/Controller?link=main_room&action=update&s_id=" + <%=bean.getS_aid()%> + "&sensor=curtain&state=" + $('#slider2').val() , success: function(result){
		        //$("#div1").html(result);
		    }});
...
<!-- 큰 방 -->
<div style="background:#fceda7; width:351px; height:247.5px; border:solid black 1px; position:relative; left:350px; top:-699px; text-align:center;">main room</div>
	</div>
